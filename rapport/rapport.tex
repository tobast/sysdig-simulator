...
Toute une batterie de tests simples se trouve déjà dans \lstbash{tests/} : on peut lancer \lstbash{./runTests tests/*} comme première vérification.
...
Chaque variable de type \lstocaml{TBit} est un \lstcpp{bool}~; chaque \lstocaml{TBitArray(p)} est un \lstcpp{std::bitset<p>} (permettant des opérations bitwise efficaces). Les mémoires (RAM et ROM) sont quant à elles des \lstcpp{vector<bool>} (à terme, il peut être envisagé d'optimiser les mémoires en fonction de leur \lstocaml{word_size}).
